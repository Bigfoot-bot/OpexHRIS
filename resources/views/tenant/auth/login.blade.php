...
<head>
    ...
    <title>{{ tenant('name') ?? 'HRIS' }} - Login</title>
...
<body class="bg-green-50 min-h-screen flex items-center justify-center">
    <div class="w-full max-w-md">
        ...
        <div class="bg-white rounded-2xl border border-green-100 p-8 shadow-sm">
            ...
            <form method="POST" action="{{ route('tenant.login.post') }}">
                ...
                <div class="flex justify-end mb-6">
                    <a href="{{ route('tenant.password.request') }}" class="text-xs text-emerald-600 hover:text-emerald-800">Forgot password?</a>
                </div>
                <button type="submit"
        ...
        <p class="text-center text-xs text-gray-400 mt-6">
            {{ tenant('name') ?? 'HRIS Platform' }} &copy; {{ date('Y') }}
        </p>
    </div>
<script>
