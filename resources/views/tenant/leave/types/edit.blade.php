...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ route('tenant.leave-types.update', $leaveType) }}">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                              class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">{{ old('description', $leaveType->description) }}</textarea>
                </div>

                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1.5">Days Allowed Per Year *</label>
                    <input type="number" name="days_allowed" value="{{ old('days_allowed', $leaveType->days_allowed) }}" required min="0"
                           class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"/>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <input type="checkbox" name="requires_document" value="1" {{ old('requires_document', $leaveType->requires_document) ? 'checked' : '' }}
                               class="w-4 h-4 text-emerald-600"/>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div>
                        ...
                        <input type="checkbox" name="allow_half_day" value="1" {{ old('allow_half_day', $leaveType->allow_half_day) ? 'checked' : '' }}
                               class="w-4 h-4 text-emerald-600"/>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div>
                        ...
                        <input type="checkbox" name="carry_forward" value="1" {{ old('carry_forward', $leaveType->carry_forward) ? 'checked' : '' }}
                               class="w-4 h-4 text-emerald-600"/>
                    </div>
                </div>
