...
@section('page-subtitle', 'Add funds to your facility wallet')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-emerald-700 rounded-2xl p-5 text-white flex items-center justify-between">
        <div>
            <p class="text-xs opacity-70">Current Balance</p>
            <p class="text-2xl font-bold">KES {{ number_format($wallet->balance, 2) }}</p>
        </div>
        <a href="{{ route('tenant.wallet.index') }}" class="text-xs opacity-70 hover:opacity-100">View History &rarr;</a>
    </div>
    ...
    <div style="display:flex; gap:24px; align-items:stretch;">
        ...
        <div style="flex:1; background:white; border-radius:16px; border:1px solid #e5e7eb; padding:24px; display:flex; flex-direction:column;">
            <div style="display:flex; align-items:center; gap:12px; margin-bottom:16px;">
                <div style="width:36px; height:36px; background:#fef3c7; border-radius:8px; display:flex; align-items:center; justify-content:center;">
                    <svg style="width:20px; height:20px; color:#d97706;" fill="none" stroke="#d97706" viewBox="0 0 24 24">
            ...
            <div style="background:#fffbeb; border-radius:10px; padding:12px; margin-bottom:16px; font-size:12px; color:#92400e;">
                <p><strong>Paybill:</strong> {{ $settings->paybill_number ?? 'Contact support' }}</p>
                <p><strong>Account No:</strong> {{ $branding->mpesa_account ?? tenant()->slug ?? tenant('id') }}</p>
            </div>
            <form method="POST" action="{{ route('tenant.wallet.submit-top-up') }}" style="display:flex; flex-direction:column; flex:1;">
                @csrf
                <input type="hidden" name="payment_method" value="mpesa_manual"/>
                <div style="flex:1;">
                    <div style="margin-bottom:12px;">
                        <label style="display:block; font-size:11px; font-weight:500; color:#4b5563; margin-bottom:4px;">Amount (KES) *</label>
        ...
        <div style="flex:1; background:white; border-radius:16px; border:1px solid #e5e7eb; padding:24px; display:flex; flex-direction:column;">
            ...
            <form method="POST" action="{{ route('tenant.wallet.submit-top-up') }}" enctype="multipart/form-data" style="display:flex; flex-direction:column; flex:1;">
                @csrf
    ...
    <div style="background:white; border-radius:16px; border:1px solid #d1fae5; padding:24px;">
        <div style="display:flex; align-items:center; gap:12px; margin-bottom:16px;">
            ...
            <div>
                ...
                <p style="font-size:11px; color:#6b7280;">Get an M-Pesa prompt — wallet credited automatically</p>
            </div>
