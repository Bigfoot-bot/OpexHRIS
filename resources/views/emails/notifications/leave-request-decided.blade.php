        ...
        <tr><td align="center">
            <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
                ...
                <tr><td style="background:#ffffff;border-radius:16px;border:1px solid #d1fae5;padding:40px;">
                    <h2 style="margin:0 0 8px;font-size:20px;color:#064e3b;text-align:center;">Leave Request {{ ucfirst($leaveRequest->status) }}</h2>
                    <p style="margin:0 0 24px;font-size:14px;color:#6b7280;text-align:center;">Hi {{ $leaveRequest->employee->full_name ?? 'there' }}, your leave request has been reviewed.</p>
                    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;background:#f0fdf4;border-radius:10px;padding:16px;">
                        ...
                        <tr><td style="font-size:13px;color:#6b7280;padding:4px 0;"><strong style="color:#064e3b;">Status:</strong> <span style="color:{{ $leaveRequest->status === 'approved' ? '#065f46' : '#dc2626' }};font-weight:600;">{{ ucfirst($leaveRequest->status) }}</span></td></tr>
                        @if($leaveRequest->rejection_reason)
                        <tr><td style="font-size:13px;color:#6b7280;padding:4px 0;"><strong style="color:#064e3b;">Reason:</strong> {{ $leaveRequest->rejection_reason }}</td></tr>
                        @endif
                    </table>
                    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
                        <tr><td align="center">
                            <a href="{{ $link }}" style="display:inline-block;background-color:#065f46;color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;padding:12px 32px;border-radius:8px;">View Details</a>
                        </td></tr>
                    </table>
